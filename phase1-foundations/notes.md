...
- **Two separate init problems, two separate fixes (`mlp_init_bn.py`).** (1) *Initial loss too high*: at init the model should be maximally uncertain → loss ≈ −log(1/27) ≈ 3.30, but random init gave ~27. Cause is the **output layer**: `randn` `W2`/`b2` make large, confident-but-wrong logits. Fix: shrink `W2 *= 0.01`, zero `b2`. The wasted early steps (the loss "hockey stick") are the model un-learning false confidence. (2) *Saturated tanh*: with big `W1`/`b1`, `pre_h` is large → `tanh` pinned at ±1 → local grad `1−tanh²` ≈ 0 → dead neurons that can't learn (measured 65% with `abs(h)>0.99`). Note `tanh` *bounds* `h` to (−1,1) regardless of `W1`, so `W1` scale can't hurt the initial *loss* (only `W2` can) — but it silently kills *gradients*. Fix: Kaiming init `std = gain/√fan_in`, gain=5/3 for tanh, fan_in=30 → ×0.304. Target saturation ~5–15%, NOT ~0 (too-small weights squash activations toward 0 and underuse the nonlinearity).
- **BatchNorm's real payoff is init-robustness, not accuracy.** Normalizing `hpreact` to unit gaussian per-batch (`(x−mean(0))/std(0)`, then learnable `bngain`/`bnbias`) keeps activations healthy *regardless* of `W1` scale — decouples "trains well" from "nailed every layer's init," which is what makes deep nets trainable. Accuracy bump here is a bonus (dev 2.17→2.11). **Gotcha that bit me:** BatchNorm uses the *current batch's* stats, so eval/sampling (no batch, or coupling across examples) needs the **running EMA** of mean/std tracked during training under `torch.no_grad()`, then used at eval. Symptom of getting it wrong: `bnmean_running`/`bnstd_running` computed but never *used* — eval forward pass silently runs on un-normalized preactivations, a distribution the net never trained on. Why nonlinearity at all: stacked linears collapse (`W2(W1 x)=(W2 W1)x`) to one linear layer; `tanh` between them is what prevents the fold.
